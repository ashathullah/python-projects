...
- GNU Make
- Tesseract OCR for local runs (`tesseract.exe`) + language data for `eng` (and `tam` for Tamil PDFs)
...
### Output format (CSV vs Excel)

Default output is Excel:
- Per PDF: `csv/<pdf_stem>.xlsx`
- Combined: `csv/final_voter_data.xlsx`

To write CSV instead:
- `python main.py --output-format csv`

### Windows: Tesseract + Tamil (`tam`)

Check languages:
- `tesseract --list-langs` (must include `eng`; Tamil PDFs require `tam`)

If `tesseract` isn’t on `PATH`, set:
- PowerShell: `$env:TESSERACT_CMD="C:\\Program Files\\Tesseract-OCR\\tesseract.exe"`

If `tam` is missing, you can use a user-local tessdata folder:
- `mkdir $env:USERPROFILE\\tessdata`
- `Copy-Item "C:\\Program Files\\Tesseract-OCR\\tessdata\\eng.traineddata" "$env:USERPROFILE\\tessdata\\"`
- Put `tam.traineddata` into `$env:USERPROFILE\\tessdata\\` and set: `$env:TESSDATA_DIR="$env:USERPROFILE\\tessdata"`
